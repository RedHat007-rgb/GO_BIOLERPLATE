...
type NewRelicConfig struct{
	...
	AppLogForwadingEnabled bool `koanf:"app_log_forwading_enabled"`	
	DistributedForwadingEnabled bool `koanf:"distributed_forwading_enabled"`
...
func DefaultObservabilityConfig() *ObservabilityConfig{
	return &ObservabilityConfig{
		...
		NewRelic: NewRelicConfig{
			...
			DistributedForwadingEnabled: true,
			DebugLogging: false,
		},
...
func (c *ObservabilityConfig) Validate() error{
	...
	if c.Logging.SlowQueryThreshold<0{
		return fmt.Errorf("logging slow_query_threshold must be non_negitive")
	}
	return nil
}
